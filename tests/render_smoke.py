...
with tempfile.TemporaryDirectory(prefix='varez-render-') as temp:
    ...
    cmd('ffmpeg','-v','error','-y','-f','lavfi','-i','testsrc2=size=640x360:rate=24','-f','lavfi','-i','aevalsrc=if(between(t\\,2\\,6)\\,0\\,0.08*(sin(2*PI*120*t)+sin(2*PI*1000*t)+sin(2*PI*6000*t))):s=48000','-t','12','-c:v','libx264','-preset','ultrafast','-pix_fmt','yuv420p','-c:a','aac',str(src))
    render.make_ass([{'word':'PRUEBA','start':.2,'end':1.4}],ass)
    render.run_ffmpeg(src,out,ass,{'start':0,'end':12,'intro_end_rel':3,'captions':True,'qa':True})
    probe=json.loads(cmd('ffprobe','-v','error','-show_entries','format=duration:stream=codec_type,width,height','-of','json',str(out)))
    assert abs(float(probe['format']['duration'])-12)<.1,probe
    assert any(s.get('width')==1080 and s.get('height')==1920 for s in probe['streams'])
    before=colorfulness(out,1);after=colorfulness(out,4)
    assert before<3 and after>20,(before,after)
    telephone=frequency_ratio(out,1);normal=frequency_ratio(out,7)
    assert telephone<normal*.5,(telephone,normal)
    print('PASS: 1080x1920, intact timeline, hard B&W → color cut, telephone → normal audio')

    teaser={'start':0,'end':12,'edit_style':'teaser','hook_start_rel':7,'hook_end_rel':12,'captions':True,'qa':True,'hook_words':[{'word':'FINAL','start':.1,'end':4.8}],'words':[{'word':'INICIO','start':.1,'end':1.0},{'word':'EEEE','start':2.1,'end':2.8},{'word':'FINAL','start':7.1,'end':11.8}]}
    timeline=render.caption_words(teaser)
    assert timeline[1]['start']==5.1 and timeline[3]['start']==12.1
    ...
    assert abs(float(probe['format']['duration'])-17)<.1,probe
    assert colorfulness(out,4.9)<3 and colorfulness(out,5.05)>20
    assert frequency_ratio(out,1)<frequency_ratio(out,6)*.5
    print('PASS: ending teaser prepended, hard color cut, phone audio, repeated captions')

    jumpcut={**teaser,'remove_pauses':True}
    segments,mapped,edited=render.body_plan(jumpcut,src)
    assert len(segments)==2 and 7.9<edited<8.3,(segments,edited)
    assert [w['word'] for w in mapped]==['INICIO','FINAL']
    timeline=render.caption_words(jumpcut,segments,mapped)
    assert timeline[1]['start']==5.1 and timeline[2]['start']<8.5
    render.make_ass(timeline,ass)
    render.run_ffmpeg(src,out,ass,jumpcut,segments)
    probe=json.loads(cmd('ffprobe','-v','error','-show_entries','format=duration','-of','json',str(out)))
    assert abs(float(probe['format']['duration'])-(edited+5))<.12,(probe,edited)
    assert colorfulness(out,4.9)<3 and colorfulness(out,5.05)>20
    print('PASS: waveform silence and EEEE filler removed; words and captions stay synchronized')

    manifest={'render_id':'fixture-attempt-2','clips':[
      {'source_index':i,'source_parts':['broken' if i==2 else 'good'],'start':0,'end':12,'output_path':f'output-{i}.mp4','output_upload_url':f'upload-{i}'} for i in [1,2,3]]}
    class Response:
    ...
    render.run_ffmpeg=lambda src,out,ass,clip,segments=None:out.write_bytes(b'fixture-render')
    published=[];done=[]
