...
async function dispatch(){const rel=await ensureRelease();await gh(`https://api.github.com/repos/${OWNER}/${REPO}/actions/workflows/${WORKFLOW}/dispatches`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ref:'main',inputs:{release_id:String(rel.id),job_id:S.jobId}})})}
...
async function prepareOne(c,i){const ff=await getFF(),dur=c.end-c.start,srcStart=Math.max(0,c.start-4),srcEnd=Math.min(S.dur,c.end+4),srcDur=srcEnd-srcStart;const audio=`/a${i}.mp3`,source=`/s${i}.mp4`;let r=await ff.exec(['-ss',String(c.start),'-t',String(dur),'-i','/input/'+S.file.name,'-vn','-ac','1','-ar','16000','-c:a','libmp3lame','-b:a','64k',audio],10*60*1000);if(r!==0)throw new Error('No pude extraer el audio del clip '+(i+1));const ab=await ff.read(audio);await ff.del(audio);const words=await groqWords(ab,i);progress(40+i*7,'Preparando fuente '+(i+1)+'/5…','Corte rápido del video original.');r=await ff.exec(['-ss',String(srcStart),'-t',String(srcDur),'-i','/input/'+S.file.name,'-map','0:v:0','-map','0:a?','-c','copy','-movflags','+faststart',source],10*60*1000);if(r!==0){r=await ff.exec(['-ss',String(srcStart),'-t',String(srcDur),'-i','/input/'+S.file.name,'-vf','scale=1920:1920:force_original_aspect_ratio=decrease','-c:v','libx264','-preset','ultrafast','-crf','20','-c:a','aac','-b:a','160k','-movflags','+faststart',source],20*60*1000);if(r!==0)throw new Error('No pude preparar el video del clip '+(i+1))}const vb=await ff.read(source);await ff.del(source);const name=`${S.jobId}-source-${String(i+1).padStart(2,'0')}.mp4`;progress(44+i*7,'Subiendo fragmento '+(i+1)+'/5…','Se guarda temporalmente en un release privado de GitHub.');await uploadAsset(name,new Blob([vb],{type:'video/mp4'}),'video/mp4');return{...c,source_offset:c.start-srcStart,question_end_rel:c.question_end>=c.start&&c.question_end<c.end?c.question_end-c.start:0,words,captions:S.settings.captions,qa:S.settings.qa,reframe:S.settings.reframe}}
async function stageAll(clips){const out=[];for(let i=0;i<5;i++)out.push(await prepareOne(clips[i],i));return out}
async function uploadManifest(clips){const manifest={version:1,job_id:S.jobId,created_at:new Date().toISOString(),category:S.category,clips};await uploadAsset(`${S.jobId}-manifest.json`,new Blob([JSON.stringify(manifest)],{type:'application/json'}),'application/json')}
async function poll(){const started=Date.now();while(Date.now()-started<45*60*1000){const a=await listAssets(),outs=a.filter(x=>x.name.startsWith(S.jobId+'-output-')&&x.name.endsWith('.mp4')).sort((x,y)=>x.name.localeCompare(y.name));progress(86+outs.length*2.5,'Editando en GitHub…',outs.length+'/5 clips terminados. Podés dejar esta pantalla abierta.');if(outs.length>=5){await showOutputs(outs.slice(0,5));progress(100,'Listo','Los cinco videos están terminados.');return}await new Promise(r=>setTimeout(r,8000))}throw new Error('El render tardó demasiado. Revisá GitHub Actions.')}
async function assetBlob(asset){const r=await fetch(asset.url,{headers:{Authorization:'Bearer '+cfg().github,Accept:'application/octet-stream','X-GitHub-Api-Version':'2026-03-10'}});if(!r.ok)throw new Error('No pude descargar '+asset.name);return r.blob()}
async function showOutputs(assets){const g=$('#grid');g.innerHTML='';for(let i=0;i<assets.length;i++){const blob=await assetBlob(assets[i]),url=URL.createObjectURL(blob),c=S.clips[i]||{};const el=document.createElement('article');el.className='clip';el.innerHTML=`<h4>${esc(c.title||'Clip '+(i+1))}</h4><p>${esc(c.reason||'')}</p>`;const v=document.createElement('video');v.controls=true;v.playsInline=true;v.src=url;el.appendChild(v);const a=document.createElement('a');a.className='download';a.href=url;a.download=`Varez_${String(i+1).padStart(2,'0')}_${(c.title||'clip').replace(/[^a-z0-9áéíóúñ_-]+/gi,'_')}.mp4`;a.textContent='Descargar MP4';el.appendChild(a);g.appendChild(el)}$('#results').style.display='block';$('#results').scrollIntoView({behavior:'smooth',block:'start'})}

$('#go').onclick=async()=>{if(!S.file)return alert('Primero elegí un video.');if(S.dur>1800)return alert('Máximo 30 minutos en esta beta.');if(!ensureConfig())return;$('#go').disabled=true;$('#results').style.display='none';S.jobId=crypto.randomUUID().replaceAll('-','').slice(0,16);try{await ensureRelease();const gf=await geminiUpload(S.file);let clips;try{clips=await geminiSelect(gf)}finally{geminiDelete(gf)}if(clips.length!==5)fail('No pude obtener cinco clips válidos.');S.clips=clips;progress(32,'Tengo los 5 momentos','Ahora preparo subtítulos y los fragmentos fuente.');const staged=await stageAll(clips);if(S.ff){S.ff.terminate();S.ff=null;S.mounted=false}await uploadManifest(staged);progress(82,'Mandando a edición remota…','GitHub FFmpeg va a producir los cinco MP4.');await dispatch();await poll()}catch(e){progress(100,'Hubo un problema',e.message||String(e));$('#log').className='log error';alert(e.message||e)}finally{$('#go').disabled=false}};
