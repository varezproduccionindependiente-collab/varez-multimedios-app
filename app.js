...
async function uploadManifest(clips){const manifest={version:1,job_id:S.jobId,created_at:new Date().toISOString(),category:S.category,clips};const up=await cloudUpload(`${S.jobId}-manifest.json`,new Blob([JSON.stringify(manifest)],{type:'application/octet-stream'}),'application/octet-stream');return up.url}
